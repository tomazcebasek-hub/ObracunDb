...
public class ObracunService
{
    ...
    /// <summary>
    /// Pridobi seštevek postavk po artiklu za doloèeno obdobje.
    /// </summary>
    ...
    /// <summary>
    /// Pridobi distinct mesece/leta iz OBRACUN_OSNUTEK_POS, razvršèene padajoèe.
    /// </summary>
    ...
    /// <summary>
    /// Pridobi seštevek postavk po partnerjih za doloèen artikel.
    /// </summary>
    ...
    /// Podatki vkljuèujejo tudi vse kar potrebuje Info panel, da ni potrebno dodatno pridobivanje.
    /// </summary>
    public async Task<List<OsnutekPartnerDto>> GetOsnutkiAsync(int leto, int mesec)
    {
        ...
        // 2. Korišèene minute po tipu iz OBRACUN_OSNUTEK_NALOG_OBRACUN
        await using (var cmd = new FbCommand(@"
            SELECT PARTNER,
    ...
    /// <summary>
    /// Pridobi raèune za partnerja (od leta 2026 naprej). Èe jeAvans=true, vrne samo tip_racuna=4, sicer vse razen tip_racuna=4.
    /// </summary>
    ...
    /// <summary>
    /// Pridobi postavke raèuna glede na TIP_RACUNA (ista logika kot v Partnerji).
    /// </summary>
    ...
    public async Task<PartnerPovzetekDto> GetPartnerPovzetekAsync(int leto, int mesec, int partner)
    {
        ...
        // 2. Korišèene minute iz OBRACUN_OSNUTEK_NALOG_OBRACUN (vsota po partnerju)
        await using (var cmd = new FbCommand(@"
            SELECT
                COALESCE(SUM(MINUTE_ODSTETE_PARTNER_MINUTE), 0),
                COALESCE(SUM(MINUTE_ODSTETE_PREDRACUN), 0),
    ...
    /// <summary>
    /// Pridobi naloge za partnerja (obraèunane ali neobraèunane)
    /// </summary>
    ...
    /// <summary>
    /// Pridobi postavke delovnega naloga (FA_DN_NALOG_KNJ) + roène vnose iz OBRACUN_OSNUTEK_POS.
    /// </summary>
    public async Task<List<PotrjevanjeNalogPozDto>> GetPostavkeNalogaAsync(string stevilka, int leto)
    {
        ...
        // 2. Roèni vnosi iz OBRACUN_OSNUTEK_POS (vezani na ta nalog)
    ...
    /// <summary>
    /// Pridobi predraèune za partnerja (status 5 ali plaèani).
    /// </summary>
    public async Task<List<PredracunGridDto>> GetPredracuniZaPartnerjaAsync(int partner, int? predMesec = null, int? predLeto = null)
    {
        ...
        // 2. Raèuni vezani na predraèune tega partnerja
        ...
        // 4. Minute iz predraèunov (postavke × OBRACUN_PAKET_MINUTE)
    ...
    /// <summary>
    /// Pridobi postavke predraèuna (FA_PREDRACUN_KNJIZBA) z nazivi artiklov.
    /// </summary>
    ...
    /// <summary>
    /// Shrani polovièno kilometrino (SIF29) v FA_DN_NALOG.
    /// </summary>
    ...
    /// <summary>
    /// Preveri in ustvari manjkajoèe OBRACUN_DN zapise za vse naloge od 1.MM.LLLL naprej.
    /// Za vsak nalog, ki nima zapisa v OBRACUN_DN, ustvari nov zapis z vrednostjo KajObracunam
    /// doloèeno iz SIF28 (0 = KmMin, 1 = Niè, ostalo = Nedefinirano).
    /// </summary>
    ...
    /// <summary>
    /// Shrani ro
    /// ZS se doloèi kot MAX(ZS)+1 za dani mesec/leto/partner.
    /// </summary>
    ...
    /// <summary>
    /// Izbriši roèno vneseno postavko iz OBRACUN_OSNUTEK_POS.
    /// </summary>
    ...
    /// <summary>
    /// Vrne množico povezanih predraèunov za nalog (iz OBRACUN_DN_PREDRACUN).
    /// </summary>
    ...
    /// <summary>
    /// Shrani spremembe povezav predraèunov z nalogom (dodaj/briši vrstice v OBRACUN_DN_PREDRACUN).
    /// </summary>
    ...
    /// <summary>
    /// Pridobi naloge za partnerja za prikaz v popup-u z obraèunskimi podrobnostmi.
    /// </summary>
    public async Task<List<NalogiPartnerNalogDto>> GetNalogiZaPartnerjaPopupAsync(int leto, int mesec, int partner)
    {
            ...
            ORDER BY ZACETEK_DATUM DESC, ZACETEK_URA DESC", connection))
        {
            ...
            while (await reader.ReadAsync())
            {
                ...
                // Izraèunaj trajanje: terenski = KonecUra - ZacetekUra
                int trajanje = 0;
                ...
                nalogi[key] = new NalogiPartnerNalogDto
                {
                    ...
                    KonecUra = konecUra,
                    Trajanje = trajanje,
                    Serviser = potnik, // zaèasno šifra, zamenjamo z imenom
                    Opis = opis
                };
            }
        }
        ...
        // 2. Za helpdesk naloge (1000000-1999999) preberi kolièino artikla 047512
        ...
        // 4. Preberi podrobnosti obraèuna iz OBRACUN_OSNUTEK_NALOG_OBRACUN
        ...
        // 5. Obogati obraèune z nazivi artiklov
    ...
    /// <summary>
    /// Pridobi seštevek dela po šifri artikla — pregled kolièin iz OBRACUN_OSNUTEK_POS
    /// z razdelitvijo korišèenja iz OBRACUN_OSNUTEK_NALOG_OBRACUN.
    /// </summary>
