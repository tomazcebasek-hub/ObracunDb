...
public class ReklamacijeService
{
        ...
        try
        {
            await using var insertHeader = new FbCommand(@"
                INSERT INTO OBRACUN_REKLAMACIJA
                    (TIP_REKLAMACIJE, PARTNER, DATUM_ZAHTEVE, STEVILKE_POGODB, KONTAKT, TIP_PREKINITVE, RACUNI_DO_DNE)
                VALUES
                    (@TipReklamacije, @Partner, @DatumZahteve, @StevilkePogodb, @Kontakt, @TipPrekinitve, @RacuniDoDne)
                RETURNING ID", connection);
            insertHeader.Transaction = transaction;

            insertHeader.Parameters.AddWithValue("@TipReklamacije", (int)TipReklamacije.PrekinitevPogodbe);
            ...
            await using var insertPos = new FbCommand(@"
                INSERT INTO OBRACUN_REKLAMACIJA_POS
                    (ID_REKLAMACIJA, DATUM, UPORABNIK, OPIS, KDO_NAJ_OBDELA)
                VALUES
                    (@IdReklamacija, @Datum, @Uporabnik, @Opis, @KdoNajObdela)", connection);
    ...
    public async Task AddPostavkaAsync(int idReklamacija, string? komentar, string? kdoNajObdela)
    {
        await using var connection = _connectionManager.GetConnection();
        await connection.OpenAsync();
